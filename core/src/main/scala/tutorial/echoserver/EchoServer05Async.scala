 ...
 */
object EchoServer05Async extends hutil.IOApp {

  def echoProtocol[F[_]: Async: ContextShift](clientSocket: Socket, stopFlag: MVar[F, Unit])(
    ...
    def loop(reader: BufferedReader, writer: BufferedWriter, stopFlag: MVar[F, Unit]): F[Unit] =
      for {
        lineE /*Either[Throwable, String]*/ <- Async[F].async {
  ...
  def serve[F[_]: Concurrent: ContextShift](serverSocket: ServerSocket, stopFlag: MVar[F, Unit])(
    ...
    for {
      socket <- Sync[F]
                 ...
                 .bracketCase { socket =>
                   startEchoFiber(socket) >> Sync[F].pure(
                     socket
                   ) // Client attended by its own Fiber, socket is returned
                 } { (socket, exit) =>
                   exit match {
