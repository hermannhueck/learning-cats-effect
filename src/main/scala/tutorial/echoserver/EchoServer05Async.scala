 ...
 */
object EchoServer05Async extends IOApp {

  println("\n-----")

  def echoProtocol[F[_]: Async: ContextShift](clientSocket: Socket, stopFlag: MVar[F, Unit])(implicit clientsExecutionContext: ExecutionContext): F[Unit] = {

    val csf = implicitly[ContextShift[F]]

    def loop(reader: BufferedReader, writer: BufferedWriter, stopFlag: MVar[F, Unit]): F[Unit] =
      for {
        lineE /*Either[Throwable, String]*/ <- Async[F].async { (cb: Either[Throwable, Either[Throwable, String]] => Unit) =>
  ...
  def server[F[_]: Concurrent: ContextShift](serverSocket: ServerSocket): F[ExitCode] = {
    ...
    implicit val clientsExecutionContext: ExecutionContext = ExecutionContext.fromExecutor(clientsThreadPool)
