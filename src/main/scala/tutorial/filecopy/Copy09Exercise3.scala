 ...
 */
object Copy09Exercise3 extends IOApp {

  // IOApp allows for purely functional code
  // provides implicit instances of Timer[IO] and ContextShift[IO]

  type Semaphore[F[_]] = MVar[F, Unit]

  object Semaphore {

    def empty[F[_]: Concurrent]: F[MVar[F, Unit]] = MVar.empty[F, Unit]

    def apply[F[_]: Concurrent]: F[MVar[F, Unit]] = MVar.of[F, Unit](())
  }

  implicit class semaphoreSyntax[F[_]: Sync](sem: Semaphore[F]) {

    def acquire: F[Unit] = sem.take

    def release: F[Unit] = sem.put(())
  ...
  override def run(args: List[String]): IO[ExitCode] =
    for {
      _           <- if (args.length < 2) IO.raiseError(new IllegalArgumentException(s"Need origin and destination files\n$usage")) else IO.unit
      _           <- IO(println("\n-----"))
      ...
      _           <- if (destCanWrite) IO.unit else IO.raiseError(new IllegalArgumentException(s"output file not writable\n$usage"))
